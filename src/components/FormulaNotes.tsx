...
const fieldClass = 'w-full rounded-md border border-line bg-surface px-3 py-2 text-[14px] text-ink placeholder:text-ink-3 disabled:opacity-60'
const buttonClass = 'rounded-md border border-line bg-surface px-3 py-2 text-[13px] font-medium text-ink-2 hover:bg-surface-sunken disabled:opacity-50'
const primaryClass = 'rounded-md bg-accent px-4 py-2 text-[13px] font-medium text-white hover:bg-accent-strong disabled:opacity-50'
  ...
  return <main className="mx-auto max-w-[100rem] space-y-5 px-4 py-6 lg:px-6">
    <header className="flex flex-wrap items-start justify-between gap-4">
      <div><h2 className="text-[20px] font-semibold">회사별 배합비 노트</h2>
        <p className="mt-1 text-[14px] text-ink-2">검색한 배합 조건과 시장 근거를 회사별로 모아 두고, 상담 내용을 이어서 관리하세요.</p>
        <p className="mt-2 text-[12px] text-ink-3">서버에 저장 · 같은 서비스 사용자와 공유</p></div>
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[21rem_minmax(0,1fr)]">
      <aside className="space-y-4 rounded-lg border border-line bg-surface p-4" aria-label="회사별 노트 목록">
        <div className="flex items-center justify-between"><h3 className="text-[14px] font-semibold">저장된 노트</h3>
          <button type="button" className="text-[13px] text-accent-strong" onClick={() => setRefresh((n) => n + 1)}>새로고침</button></div>
        <label className="block text-[13px] text-ink-2">회사 선택
        ...
        {error ? <p role="alert" className="rounded-md bg-danger-soft p-3 text-[13px] text-danger">{error}</p> : null}
        {loading ? <p role="status" className="py-6 text-center text-[13px] text-ink-3">목록을 불러오는 중…</p>
          : !notes.length && !error ? <p className="py-8 text-center text-[13px] text-ink-3">{query || company ? '조건에 맞는 노트가 없습니다.' : '첫 번째 회사 노트를 저장해 보세요.'}</p>
          : <ul className="max-h-[32rem] space-y-2 overflow-y-auto">{notes.map((note) => <li key={note.id}>
            <button type="button" disabled={opening} onClick={() => openNote(note.id)} aria-pressed={editor.note?.id === note.id}
              ...
              <span className="text-[12px] font-medium text-accent-strong">{note.company}</span>
              <strong className="mt-1 block text-[14px] font-medium keep-all">{note.title}</strong>
              <span className="mt-2 block text-[12px] text-ink-3">{dateLabel(note.updatedAt)} 수정</span>
            </button>
          </li>)}</ul>}
      ...
      <div className="min-w-0">
        {opening ? <p role="status" className="p-4 text-[13px] text-ink-3">노트를 여는 중…</p> : null}
    ...
    <div className="rounded-lg border border-line bg-surface p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div><h3 className="text-[16px] font-semibold">{note ? (editing ? '노트 수정' : note.title) : '새 배합비 노트'}</h3>
          {note ? <p className="mt-1 text-[13px] text-ink-3">{note.company} · {dateLabel(note.updatedAt)} 서버 저장</p> : null}</div>
      ...
      {editing ? <form onSubmit={save} className="space-y-4">
        <fieldset disabled={busy} className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-[13px] text-ink-2">회사명 <span className="text-accent">*</span>
            ...
            <label className="text-[13px] text-ink-2">노트 제목 <span className="text-accent">*</span>
          ...
          <label className="block text-[13px] text-ink-2">브리핑 붙여넣기 <span className="text-accent">*</span>
          ...
          {draft.sourceText ? <p role="status" className={`text-[13px] ${parsed ? 'text-accent-strong' : 'text-danger'}`}>{parsed ? '브리핑을 인식했습니다. 아래에서 정리된 내용을 확인하세요.' : '브리핑 형식을 인식하지 못했습니다. ‘텍스트 복사’한 내용 전체를 붙여넣어 주세요.'}</p> : null}
          <label className="block text-[13px] text-ink-2">상담 메모
      ...
      </form> : note?.memo ? <div className="rounded-md bg-surface-sunken p-4"><h4 className="text-[13px] font-medium text-ink-2">상담 메모</h4><p className="mt-2 whitespace-pre-wrap break-words text-[14px] leading-6">{note.memo}</p></div> : null}
      {error ? <p role="alert" className="mt-3 rounded-md bg-danger-soft p-3 text-[13px] text-danger">{error}</p> : null}
      {message ? <p role="status" className="mt-3 text-[13px] text-accent-strong">{message}</p> : null}
    </div>
    <FormulaNotePreview text={draft.sourceText} />
    {draft.sourceText ? <details className="rounded-lg border border-line bg-surface p-4">
      <summary className="cursor-pointer text-[13px] font-medium text-ink-2">복사 원문 확인</summary>
      <pre className="mt-3 whitespace-pre-wrap break-words font-sans text-[13px] leading-6 text-ink-2">{draft.sourceText}</pre>
    </details> : null}
  </section>
}
