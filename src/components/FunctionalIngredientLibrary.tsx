...
const categories = ['all', 'notified', 'recognized'] as const
...
const sourceButtonClass = 'inline-flex w-fit max-w-full items-center justify-center rounded-md border border-[#40566e] bg-[#40566e] px-3 py-1.5 text-[15px] leading-5 font-semibold text-white transition-colors hover:border-[#34465b] hover:bg-[#34465b]'
  ...
  return (
    <div className="space-y-2">
      ...
      {!standard.intakes.length ? <div>
        <p className="text-[14px] font-medium text-ink-2">일일섭취량 기재 없음</p>
...
function IngredientDetails({ ingredient }: { ingredient: FunctionalIngredient }) {
  return (
    <div className="space-y-4 p-4 lg:p-5">
      ...
      {ingredient.review ? <section aria-label="원료 재검토 결과" className="rounded-lg border border-line bg-surface p-4">
        <h4 className="text-[14px] font-semibold text-ink">{ingredient.review.label}</h4>
        {ingredient.review.relatedRecognition ? <p className="mt-1 text-[13px] text-ink-2">관련 현행 인정번호: {ingredient.review.relatedRecognition}</p> : null}
        <ul className="mt-2 space-y-1">{ingredient.review.sources.map(source => <li key={source.url}>
          <a href={source.url} target="_blank" rel="noopener noreferrer" className={`${linkClass} text-[13px]`}>{source.label} ↗<span className="sr-only"> (새 창)</span></a>
        </li>)}</ul>
      </section> : null}
    ...
    <main className="mx-auto max-w-[104rem] space-y-5 px-4 py-5 lg:px-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex max-w-full flex-wrap items-start gap-2">
          <a className={sourceButtonClass} href={INGREDIENT_SOURCES.codex} target="_blank" rel="noopener noreferrer">건강기능식품공전 (바로가기)<span className="sr-only"> (새 창)</span></a>
          <a className={sourceButtonClass} href={INGREDIENT_SOURCES.search} target="_blank" rel="noopener noreferrer">식품안전나라 원료 검색 (바로가기)<span className="sr-only"> (새 창)</span></a>
        </div>
      </div>
      ...
      <p className="text-[13px] leading-6 text-ink-2">
        현재 기준이 확인된 고시형·개별인정형 원료만 표시합니다. 섭취량은 <strong>하루 기준</strong>이며 원료·지표성분·균수 등 표시된 기준을 따릅니다.
      </p>
                  ...
                  <Fragment key={ingredient.id}>
                    <tr className="border-t border-line align-top">
                      <th scope="row" className="px-4 py-3 font-normal">
                        <span className="mb-1 inline-block rounded bg-accent-soft px-2 py-0.5 text-[12px] text-accent-strong">{ingredientCategoryLabels[ingredient.category]}</span>
                        <p className="keep-all text-[14px] font-semibold leading-6 text-ink">{ingredient.name}</p>
                        {ingredient.review ? <p className="mt-1 text-[12px] text-ink-3">{ingredient.review.label}</p> : null}
                      ...
                      <td className="keep-all px-4 py-3 text-[13px] leading-6 text-ink-2">
                        {ingredient.category === 'notified' ? first?.functionality : purposes.length ? purposes.join(' / ') : first?.functionality}
                        {ingredient.category === 'recognized' ? <p className="mt-2 text-[12px] text-ink-3">인정번호별 기능성은 상세에서 확인</p> : null}
                      </td>
                      <td className="px-4 py-3">
                        {first ? <StandardIntakes standard={first} compact /> : null}
      ...
      <details className="rounded-lg border border-line bg-surface p-4 text-[13px] leading-6 text-ink-2">
        ...
        <p className="mt-2">I-0040 773행, I-0050 447행과 기존 원료 목록 {ingredientAudit.legacyRows}행을 대조하고 현행 기준이 확인된 고시형 {ingredientAudit.counts.notified}개, 개별인정형 {ingredientAudit.counts.recognized}개를 수록했습니다.</p>
        <p className="mt-2">식품안전나라 원료 게시물 655건, 식약처 종합정보 원료 목록 699건과 개별인정 상세 465건을 대조했습니다.</p>
