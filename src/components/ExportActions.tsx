...
import { readLogo } from '@/lib/export/logo'
...
export function ExportActions({ briefing: original, disabled, freshness }: Props) {
  ...
  return (
    <div className="relative flex w-full shrink-0 flex-wrap items-center justify-end gap-2 sm:w-auto">
      {message ? (
        <p aria-live="polite" className="w-full text-right text-[11px] text-ink-3">
          {message}
        </p>
      ) : null}

      <button type="button" aria-expanded={settingsOpen} onClick={() => setSettingsOpen(!settingsOpen)} className="rounded-md border border-line px-3 py-1.5 text-[12px] text-ink-2">내보내기 설정{customer ? ' · 고객용' : ''}</button>
      {settingsOpen ? <div className="absolute right-0 top-full z-50 mt-2 max-h-[70dvh] w-[min(22rem,calc(100vw-2rem))] overflow-y-auto rounded-lg border border-line bg-surface p-4 text-[12px] text-ink-2 shadow-lg" aria-label="내보내기 설정">
        ...
        <label className="mt-3 block">회사 로고 (PNG·JPG, 2MB 이하)
          <input type="file" accept="image/png,image/jpeg" disabled={busy !== null} className="mt-2 w-full text-[11px]" onChange={event => {
            const file = event.target.files?.[0]; event.target.value = ''
            if (file) void run('logo', async () => { setLogo(await readLogo(file)); setPreview(null); return '로고를 적용했습니다.' })
          }} />
        </label>
        {logo ? <div className="mt-2 flex items-center gap-3">
          ...
          <button type="button" className="underline" onClick={() => { setLogo(null); setPreview(null) }}>로고 제거</button>
        </div> : null}
        <p className="mt-2 text-[11px] text-ink-3">로고는 이번 화면에서 내보내는 PDF·이미지에 적용됩니다.</p>
        ...
        <p className="mt-2 text-[11px] leading-4 text-ink-3">기본으로 숨기는 항목은 없습니다. 고객용 보기를 켜면 아래에서 선택한 항목만 모든 내보내기에서 숨깁니다. 검색 결과 수치는 유지됩니다.</p>
        ...
        <p className="mt-2 text-[11px] text-ink-3">자유 검색어에 입력한 회사명 등 다른 텍스트는 자동으로 가리지 않습니다. 미리보기에서 확인해 주세요.</p>
...
function ExportPreview({ preview, onClose }: { preview: { image: string; text: string }; onClose: () => void }) {
  ...
  return <dialog ref={dialogRef} onCancel={onClose} className="fixed inset-0 m-auto max-h-[calc(100dvh-2rem)] w-[calc(100vw-2rem)] max-w-3xl overflow-hidden rounded-lg bg-surface p-4 backdrop:bg-ink/40" aria-label="내보내기 미리보기">
    <div className="flex items-center justify-between"><strong className="text-[13px]">내보내기 미리보기</strong><button type="button" autoFocus className="rounded border border-line px-3 py-1 text-[12px]" onClick={onClose}>미리보기 닫기</button></div>
    <div className="mt-3 max-h-[calc(100dvh-7rem)] overflow-auto">
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={preview.image} alt="로고와 고객용 설정이 반영된 브리핑" className="w-full" />
      <details className="mt-3 text-[12px]"><summary>텍스트 복사 내용 확인</summary><pre className="mt-2 whitespace-pre-wrap break-words">{preview.text}</pre></details>
    </div>
  </dialog>
}
