  ...
  return (
    <div>
      ...
      <div className="mt-2 flex items-center gap-2">
        <label htmlFor="encoding" className="text-[12px] text-ink-3">
          인코딩
        </label>
        <select
          id="encoding"
          ...
          className="flex-1 rounded-md border border-line bg-surface px-2 py-1 text-[12px] text-ink-2"
        >
      ...
      {status.phase === 'error' ? (
        <p className="mt-3 rounded-md border border-danger/30 bg-danger-soft px-2.5 py-2 text-[12px] leading-4 text-danger keep-all">
          {status.message}
        </p>
      ) : null}
      ...
      {status.phase === 'done' ? (
        <div className="mt-3 rounded-md border border-line bg-surface-sunken px-2.5 py-2">
          <p className="text-[12px] leading-4 text-ink-2 keep-all">
          ...
          <button
            ...
            className="mt-1 text-[12px] text-ink-3 underline-offset-2 hover:text-ink hover:underline"
          >
          ...
          {showDetail ? (
            <dl className="mt-2 flex flex-col gap-1 border-t border-line pt-2">
              {Object.entries(status.report.columnMap).map(([source, target]) => (
                <div key={source} className="flex items-start justify-between gap-2 text-[12px]">
      ...
      {verifyNote ? (
        <p className="mt-2 rounded-md border border-line bg-surface-sunken px-2.5 py-2 text-[12px] leading-4 text-ink-2 keep-all">
          {verifyNote}
        </p>
      ) : null}

      {saveStatus.phase === 'error' ? (
        <p className="mt-2 rounded-md border border-danger/30 bg-danger-soft px-2.5 py-2 text-[12px] leading-4 text-danger keep-all">
