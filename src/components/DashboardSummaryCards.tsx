    ...
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="시장 배합비 핵심 지표">
        ...
        <div className="rounded-lg border border-accent/25 bg-surface px-5 py-4">
          ...
          <p className="mt-1 text-[12px] leading-4 text-ink-2">{empty ? '데이터 없음' : '권장량 100% 이상인 제품의 비율'}</p>
          <p className="mt-1 text-[12px] leading-4 text-ink-3">함량 확인 {formatInt(content.productCount)}건 · 권장량 비교 가능 {formatInt(content.comparableCount)}건</p>
          <label className="mt-2 block text-[12px] text-ink-3">권장량 비교 대상
      ...
      <div className="flex flex-wrap items-center gap-x-6 gap-y-2 rounded-lg border border-line bg-surface px-5 py-3 text-[13px] text-ink-2" aria-label="시장 참여 지표">
