 ...
 *  2) 등급: 목록을 메이저 제조사부터 보여주기 위한 순위. 공장이 달라도 등급은
 *     모기업 기준으로 같이 움직여, 한 그룹의 공장들이 목록에서 흩어지지 않는다.
 */
...
/**
 * 같은 법인이 데이터 안에서 두 가지로 적힌 경우만 한쪽으로 모은다.
 * 키는 `compact()` 결과(법인격을 뗀 알맹이), 값은 법인격 표기다.
 * 근거는 (a) 원본 데이터 안의 다른 표기, (b) 공개된 등기 상호다.
 */
const CORPORATE_FORMS: Record<string, string> = {
  // 원본에 `주식회사한미양행`(1,684건)과 `(주)한미양행 선유공장`(160건)이 함께 있다.
  한미양행: '(주)한미양행',
  // `주식회사 락토메이슨`과 `(주)락토메이슨 제3공장`이 함께 있다.
  락토메이슨: '(주)락토메이슨',
  // `고려인삼과학주식회사`와 `고려인삼과학(주) 2공장`이 함께 있다.
  고려인삼과학: '고려인삼과학(주)',
  // 법인격이 아예 빠진 표기가 섞여 있다.
  신비바이오: '(주)신비바이오',
  다원바이오: '주식회사 다원바이오',
  다정: '(주)다정',
  뉴트라젠: '(주)뉴트라젠',
  비티씨: '주식회사 비티씨',
  // `고려은단 헬스케어(주)`와 `고려은단헬스케어(주)`가 함께 있다.
  고려은단헬스케어: '고려은단헬스케어(주)',
}
