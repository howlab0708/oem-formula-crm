...
import { allowanceLabel, blank, unitNoun, type Totals } from './calc'
...
export function materialSheetToText(spec: PackagingSpec, totals: Totals): string {
  ...
  const rows = totals.materials.map((item) => [
    item.row.name,
    ...
    item.batchKg,
    item.row.usage, // 자동 계산 중인 칸은 비워 재붙여넣기 때 kg 고정값으로 바뀌지 않게 한다.
    item.row.unitPrice,
    item.amount,
    item.row.packKg,
    item.row.note,
