...
/** 회사 표를 다루는 모든 함수는 이 경로로만 DB 에 닿는다(`getSql` 을 직접 쓰지 않는다). */
...
export async function deleteSavedSearch(id: string, owner: string) {
  const rows = await query(sql => sql`delete from saved_searches where id=${id} and owner_key=${owner} returning id`)
